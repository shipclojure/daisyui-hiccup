...
(defn scene-container [& children]
  [:div.flex.flex-row.items-center.gap-3 children])
...
(def x-icon
  [:svg {:width "16" :height "12" :viewBox "0 0 300 271" :xmlns "http://www.w3.org/2000/svg"}
...
(def apple-icon
  [:svg {:width "16" :height "16" :xmlns "http://www.w3.org/2000/svg" :viewBox "0 0 1195 1195"}
...
(def amazon-icon
  [:svg {:width "16" :height "16" :xmlns "http://www.w3.org/2000/svg" :viewBox "0 0 16 16"}
   [:g {:fill "black"}
    [:path {:d "M14.463 13.831c-1.753 1.294-4.291 1.981-6.478 1.981-3.066 0-5.825-1.131-7.912-3.019-.163-.147-.019-.35.178-.234 2.253 1.313 5.041 2.1 7.919 2.1 1.941 0 4.075-.403 6.041-1.238.294-.125.544.197.253.409z"}]
...
(def metamask-icon
  [:svg {:width "16" :height "16" :xmlns "http://www.w3.org/2000/svg" :viewBox "0 0 507.83 470.86"}
   [:defs {}
    [:style {} ".b,.c,.d,.e,.f,.g,.j{stroke-linecap:round;stroke-linejoin:round}.b{fill:#e4761b;stroke:#e4761b}.c{fill:#d7c1b3;stroke:#d7c1b3}.d{fill:#233447;stroke:#233447}.e{fill:#cd6116;stroke:#cd6116}.f{fill:#e4751f;stroke:#e4751f}.g{fill:#f6851b;stroke:#f6851b}.j{fill:#763d16;stroke:#763d16}"]]
   [:path {:d "M482.09.5 284.32 147.38l36.58-86.66z" :style "fill:#e2761b;stroke:#e2761b;stroke-linecap:round;stroke-linejoin:round"}]
   [:path {:d "m25.54.5 196.18 148.27-34.79-88.05zM410.93 340.97l-52.67 80.7 112.7 31 32.4-109.91zM4.67 342.76l32.2 109.91 112.7-31-52.67-80.7z" :class "b"}]
   [:path {:d "m143.21 204.62-31.41 47.51 111.9 4.97-3.97-120.25zM364.42 204.62l-77.51-69.16-2.59 121.64 111.71-4.97zM149.57 421.67l67.18-32.8-58.04-45.32zM290.88 388.87l67.38 32.8-9.34-78.12z" :class "b"}]])

(defscene button-default
  :title "Default button"
  [::ui/btn "Hello"])

(defscene button-sizes
  :title "Button sizes"
  (scene-container
   (for [[size name] [[::ui/xs "Tiny"] [::ui/sm "Small"] [nil "Default"] [::ui/lg "Large"] [::ui/xl "XL Large"]]]
     [::ui/btn {::ui/size size}
      name])))

(defscene button-colors
  :title "Button  colors"
  (scene-container
   (for [[color name] [[::ui/info "Info"] [::ui/primary "Primary"] [::ui/secondary "Secondary"] [::ui/accent "Accent"] [::ui/success "Success"] [::ui/warning "Warning"] [::ui/error "Error"] [::ui/ghost "Ghost"]]]
     [::ui/btn {::ui/color color}
      name])))

(defscene outline-buttons
  :title "Outline Buttons"
  (scene-container
   (for [[color name] [[::ui/info "Info"] [::ui/primary "Primary"] [::ui/secondary "Secondary"] [::ui/accent "Accent"] [::ui/success "Success"] [::ui/warning "Warning"] [::ui/error "Error"] [::ui/ghost "Ghost"]]]
     [::ui/btn {::ui/color color
                ::ui/variant ::ui/outline}
      name])))

(defscene soft-buttons
  :title "Soft Buttons"
  (scene-container
   (for [[color name] [[::ui/info "Info"] [::ui/primary "Primary"] [::ui/secondary "Secondary"] [::ui/accent "Accent"] [::ui/success "Success"] [::ui/warning "Warning"] [::ui/error "Error"] [::ui/ghost "Ghost"]]]
     [::ui/btn {::ui/color color
...
(defscene active-buttons
  ...
  (scene-container
   (for [[color name] [[::ui/info "Info"] [::ui/primary "Primary"] [::ui/secondary "Secondary"] [::ui/accent "Accent"] [::ui/success "Success"] [::ui/warning "Warning"] [::ui/error "Error"] [::ui/ghost "Ghost"]]]
     [::ui/btn {::ui/color color
                ::ui/variant ::ui/active}
      name])))

(defscene wide-button
  [::ui/btn {::ui/wide? true} "Wide button"])

(defscene loading-button
  :title "Loading buttons"
  (scene-container
   [::ui/btn {::ui/loading? true}]
   [::ui/btn {::ui/loading? true} "Loading..."]))

(defscene link-button
  :title "Link variant"
  [::ui/btn {::ui/variant ::ui/link}
   "Click me"])

(defscene disabled-button
  :title "Disabled button"
  [::ui/btn {::ui/disabled? true}
   "Disabled"])

(defscene button-shapes
  :title "Button shapes"
  (scene-container
...
(defscene brand-login-buttons
  ...
  [:div.flex.flex-col.gap-4
   ...
   [::ui/btn {::ui/start-icon email-icon
              :class "bg-white text-black border-[#e5e5e5]"}
    "Login with Email"]

   [::ui/btn {::ui/start-icon github-icon
              :class "bg-black text-white border-black"}
    "Login with GitHub"]

   [::ui/btn {::ui/start-icon google-icon
              :class "bg-white text-black border-[#e5e5e5]"}
    "Login with Google"]

   [::ui/btn {::ui/start-icon facebook-icon
              :class "bg-[#1A77F2] text-white border-[#005fd8]"}
    "Login with Facebook"]

   [::ui/btn {::ui/start-icon x-icon
              :class "bg-black text-white border-black"}
    "Login with X"]

   [::ui/btn {::ui/start-icon kakao-icon
              :class "bg-[#FEE502] text-[#181600] border-[#f1d800]"}
    "카카오 로그인"]

   [::ui/btn {::ui/start-icon apple-icon
              :class "bg-black text-white border-black"}
    "Login with Apple"]

   [::ui/btn {::ui/start-icon amazon-icon
              :class "bg-[#FF9900] text-black border-[#e17d00]"}
    "Login with Amazon"]

   [::ui/btn {::ui/start-icon microsoft-icon
              :class "bg-[#2F2F2F] text-white border-black"}
    "Login with Microsoft"]

   [::ui/btn {::ui/start-icon line-icon
              :class "bg-[#03C755] text-white border-[#00b544]"}
    "LINEでログイン"]

   [::ui/btn {::ui/start-icon linkedin-icon
              :class "bg-[#0967C2] text-white border-[#0059b3]"}
    "Login with LinkedIn"]

   [::ui/btn {::ui/start-icon wechat-icon
              :class "bg-[#5EBB2B] text-white border-[#4eaa0c]"}
    "Login with WeChat"]

   [::ui/btn {::ui/start-icon metamask-icon
              :class "bg-white text-black border-[#e5e5e5]"}
    "Login with MetaMask"]])
